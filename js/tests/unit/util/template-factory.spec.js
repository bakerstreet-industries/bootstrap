...
describe('TemplateFactory', () => {
  ...
  describe('Content', () => {
    ...
    it('if content is element, "config.html=true", and is same with templateElement, should not append', () => {
      fixtureEl.innerHTML = '<div id="foo">foo</div>'
      const contentElement = fixtureEl.querySelector('div')

      const factory = new TemplateFactory({
        html: true,
        template: contentElement.outerHTML,
        content: { '#foo': contentElement }
      })

      const fooEl = factory.toHtml()
      expect(fooEl.outerHTML).toEqual(contentElement.outerHTML)
    })
  })
  ...
  describe('getContent', () => {
    ...
    it('should filter empties', () => {
      ...
      expect(factory.getContent()).toEqual(['bar'])
    })
  })
